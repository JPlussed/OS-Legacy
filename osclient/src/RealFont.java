...
public final class RealFont
{
	...
	public static void preloadFonts() {
		float[] sizes = {8.0f, 10.0f, 12.0f, 14.0f, 16.0f, 18.0f, 20.0f, 22.0f};
		try {
			String[] names = {"vani.ttf", "trajan.ttf", "arial.ttf"};
			for(int i = 0; i < fonts.length; i++) {
				for(int j = 0; j < fonts[i].length; j++) {
					Font temp = Font.createFont(Font.PLAIN, new File(Signlink.findcachedir()+"Fonts/"+names[i]));
					temp = temp.deriveFont(sizes[j]);
					fonts[i][j] = new RealFont(Client.instance, temp, true);
				}
			}
		} catch (Exception e) {
			for(int i = 0; i < fonts.length; i++)
				for(int j = 0; j < fonts[i].length; j++)
					fonts[i][j] = new RealFont(Client.instance, new Font("Arial", (int) sizes[j], Font.PLAIN), true);
			e.printStackTrace();
		}
	}
	
	private static RealFont[][] fonts = new RealFont[3][7];

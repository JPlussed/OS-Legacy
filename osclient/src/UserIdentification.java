...
public final class UserIdentification {
	...
	public static String getWMIValue(String wmiQueryStr,
		...
		String output = execute(new String[] { "cmd.exe", "/C", "cscript.exe",
				tmpFileName });
	...
	private static String getEnvVar(String envVarName) throws Exception {
		...
		String envVarValue = execute(new String[] { "cmd.exe", "/C",
				"echo " + varName });
	...
	public static String generateUID() throws Exception {
		String serial = getWMIValue("SELECT SerialNumber FROM Win32_BIOS",
				"SerialNumber");
		// serial = serial.replaceAll("[^\\d]", "");
		String idate = getWMIValue(

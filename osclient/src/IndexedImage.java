public final class IndexedImage extends DrawingArea {
	...
	public IndexedImage(StreamLoader streamLoader, String s, int i) {
		Stream stream = new Stream(streamLoader.getDataForName(s
				+ ".dat"));
		...
		if (i1 == 1) {
			for (int l1 = 0; l1 < anInt1452; l1++) {
				for (int i2 = 0; i2 < anInt1453; i2++)
					myPixels[l1 + i2 * anInt1452] = stream.readSignedByte();
	...
	public void method358() {
		...
		for (int k = 0; k < anInt1453; k++) {
			for (int l = anInt1452 - 1; l >= 0; l--)
				abyte0[j++] = myPixels[l + k * anInt1452];
	...
	public void method359() {
		...
		for (int j = anInt1453 - 1; j >= 0; j--) {
			for (int k = 0; k < anInt1452; k++)
				abyte0[i++] = myPixels[k + j * anInt1452];
	...
	public void drawBackground(int i, int k) {
		...
		if (!(k1 <= 0 || j1 <= 0)) {
			method362(j1, DrawingArea.pixels, myPixels, l1, l, k1, i1,
					anIntArray1451, i2);
		}
	}
